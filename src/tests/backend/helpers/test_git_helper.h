...
TEST(GitHelper, Free) {
    // Initialise signature and buffer members, they're simplest to
    // test with.
    GitHelper git;
    git_signature_new(&git.sig, "LOOT", "loot@placeholder.net", 0, 0);
    EXPECT_NE(nullptr, git.sig);
